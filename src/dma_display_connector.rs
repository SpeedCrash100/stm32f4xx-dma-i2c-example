...
pub struct DMAI2cInterface<I2C: I2CMasterWriteDMA + 'static> {
        ...
        match cmd {
            DataFormat::U8(slice) => {
                ...
                let res = nb::block!(critical_section::with(|cs| {
                    ...
                    unsafe {
                        i2c.write_dma(I2C_ADDRESS, &self.command_buffer[..=slice.len()], None)
                    }
                }));
    ...
    fn send_data(&mut self, buf: DataFormat<'_>) -> Result<(), DisplayError> {
        match buf {
            DataFormat::U8(slice) => {
                ...
                let _res = nb::block!(critical_section::with(|cs| {
                    let mut i2c = self.i2c.borrow(cs).borrow_mut();
                    unsafe {
